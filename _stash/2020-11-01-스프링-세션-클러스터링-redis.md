...
스프링 세션을 프로젝트에 적용하는 방법은 공식 가이드의 설명(https://docs.spring.io/spring-session/docs/current/reference/html5)으로 대체하고, 이 포스트에서는 스프링 세션이 무엇인지 왜 스프링 세션REDIS를 사용했는지, 스프링 세션 REDIS 에서 세션을 어떠한 자료구조로 저장을 하는 지에 대한 분석한 내용을 정리한다. 
...
스프링 세션을 사용해야하는 근본적인 니즈는 크게 2개로 볼수 있다.

- WAS 스케일아웃이 필요한데, 서비스 관점에서 물리적으로 분리 된 WAS 들의 세션을 하나의 클러스터링으로 묶어야할 경우

- SSO 의 목적으로 하나의 웹서버에서 로그인하면 다른 웹서버들도 같이 로그인되는 효과를 원할 경우

나의 경우는 첫번째 이유가 가장컸다. 현재 재직한 게임 회사는 게임마다 백오피스와 CMS 들이 존재하고, Oauth2 기반의 SSO 시스템을 구성하여 SSO는 이용하고 있었다. 문제는 게임 규모가 커지면서 수직 확장으로 커버하던 것이 한계에 도달했고 수평 확장을 해야했는 데, 이때 서버들이 물리적으로 분산이 되니 세션이 각각 따로 놀면서 갈리는 것을 어떻게 할 것인지에 대한 고민이 있었다.  

자사에서는 당연히도 스프링 프레임워크를 사용하고 있었고, 위 니즈에 대한 해결책으로 스프링 세션이라는 모듈을 통해 해소할수 있는 기틀이 있었다. 스프링 세션은 웹 세션이라는 개념을 어디에 저장하고, 어떻게 이를 공유할 것인가에 관심을 두는 모듈이다. Mysql 과 같은 RDBMS 에도 저장할수 있고, REDIS 와 같은 캐싱을 목적으로한 NOSQL에도 저장할수 있다. 또한 웹 세션을 사용하는 스프링시큐리티에서 보안이라는 개념의 세션도 함께 사용할수 있다.


![](images/15356462.png)

스프링 세션은 서블릿 스펙의 준수 하에서, WAS 내의 서블릿의 필터를 통해 세션을 클러스터링한다. 위 이미지를 보면 알수있듯이 세션이라는 개념 자체는 전통적인 서블릿 세션 인터페이스를 기반으로 확장하여 구성하는 식을 사용한다.

위 도표에서 핵심 키워드는 SessionRepository 를 통해서 세션에 관심이 있는 여러 모듈들과의 결합을 지원한다. 나는 스프링 시큐리티를 통해 웹 서비스의 보안을 구성하고 있었는 데, 스프링 세션과 스프링 시큐리티가 위 SessionRepository 를 통해서 연결이 가능했다.

아래는 직접 스프링 세션과 스프링 시큐리티 모듈 간의 상관 관계를 분석해본 클래스 다이어그램이다.

![](images/a888140b.png)

기본적인 웹 트랜잭션 상호 작용은 아래와 같다. 여기서는 상속과 의존에 대한 것을 배제하고, 위 도표를 웹 트랜잭션 상호 작용 측면에서 흐름 순으로만 나열해본다.
...
스프링 시큐리티에서 세션이란 개념은 SessionRegistry 라는 핵심 인터페이스가 있고, 스프링 세션은 Spring Repository 를 사용하고 있다.

이 2개의 모듈들은 FindByIndexNameSessionRepository 에 의해서 SessionRegistry 와 SessionRepository 를 연결하게 되어 상호작용할수 있는 구성이다.


아키텍처의 자세한 소개는 아래 문서를 참고하길 바란다.
...
이 버킷은 스프링 시큐리티에서 자격 증명의 이름으로 검색할때 세션을 찾기 위한 인덱스 용도로 사용이 된다.
...
위 데이터를 보면 jhun 이라는 사용자 계정명을 자격증명으로 사용하고 있으며, jhun 이라는 계정명의 세션ID는 f14f10d2-4c9a-4e6a-91ec-f84bd693670b 임을 알수 있다.
...
마지막으로 세션이 활성화 된 시간을 말한다. 활성화 되는 기준은 서블릿에서 Session.getSession() 메소드를 호출했을 때의 시간이 lastAccessedTime 이 갱신 된다. 
...
이는 키가 삭제되면 발생하는 이벤트를 구독하려는 데, 이에 대한 구독 기능을 사용 할 수가 없어서 발생하는 에러이다. 클라우드 벤더에서 이 기능을 비활성화 하여서 생겨난 것이다. 즉, 설치형 REDIS 에서는 이슈가 없지만, AWS 나 Azure 의 PAAS 를 사용하게 되면 마주할수 있는 이슈이다. 
