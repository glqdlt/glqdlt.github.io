...
스프링 세션은 톰캣과 같은 표준 서블릿 컨테이너 위에서 동작한다. 서블릿 컨테이너에 Http 요청이 흘러 들어오면 몇몇 필터들이 동작을 하게 되는 데, 스프링 세션은 SessionRepositoryFilter 라는 필터를 구성하여 유휴 세션을 체크하는 등의 여러가지 세션 기능을 수행한다.

또한 필터 외에도 SessionRepository 를 통해서 세션에 관심이 있는 여러 모듈들과의 결합을 지원하며, 스프링 시큐리티와 같은 보안 모듈들도 연계가 가능 하다.

아래는 직접 스프링 세션과 스프링 시큐리티 모듈 간의 상관 관계를 분석해본 클래스 다이어그램이다.

![](images/a888140b.png)

기본적인 웹 트랜잭션 상호 작용은 아래와 같다. 여기서는 상속과 의존에 대한 것을 배제하고, 위 도표를 웹 트랜잭션 상호 작용 디버깅 측면의 흐름 순으로만 나열해본다.
...
스프링 시큐리티에서 세션이란 개념은 SessionRegistry 라는 핵심 인터페이스가 있고, 스프링 세션은 Spring Repository 를 사용하여 상호작용 하게 되는 것을 알수 있다.
