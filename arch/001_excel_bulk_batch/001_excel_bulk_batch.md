...
살펴 보면 에릭에반스의 DDD 에 따른 유사도와 같다.  헥사고날 아키텍처의 포트 & 어뎁터 패턴과도 같다.

재밌는 것은 main_topic 이 다른 node 의 topic 에 전달해주는 것인데, 보통은 main_topic 을 구독하는 컴포넌트를 만들어야한다. 다만 azure service bus topic forward 로 구글링을 하면 아래 처럼 최근에 azure 에 신기능으로 추가가 되었다.
...
### 로그를 각기 저장하고 중앙 로그 저장소로 보내는 이유

파편화의 장점은 커플링이 없어지는 것이다. 다만 파편화되면 저기에서도 확인하고 여기서도 확인하고 사람은 한눈에 보고싶어진다. 한눈에 보고싶다는 니즈가 변질되면 애초에 한곳으로 물리적으로 모아버리자 라고 커플링을 다시 만드려는 움직임이 보이기도 한다. 이건 잘못된 생각이다. 한눈에 보고싶은 부분에 대한 니즈만 해소해야지 다시 과거로 회귀하는 것은, 지금 MSA 를 도입하려고 했던 이유를 몇년뒤에 다시 만나게 할 뿐이다. 

방법은 2가이다. 각 db 를 조회해주는 서비스 주체를 만들거나, 하나의 공유 db에 저장을 해두는 것이다. 전자의 경우 보단 후자를 추천한다. 전자로 할 경우 성능적인 부분에서 이점이 없다. 자세한 얘기를 하기엔 당연한 얘기를 많이 해야하니 얘기하지는 않겠다. 후자의 경우는 또 2가지로 나뉜다.

공유 저장 NODE 에서 각 DB로 접근해서 가져오기, 각 서비스 NODE 에서 공유DB로 발송해주기

전자와 후자의 경우 장,단점이 나뉘는 데.. 아키텍처를 보면 알겠지만 필자는 후자의 방법으로 택했다. 후자의 방법으로 한 이유는 명확하다, 공유DB를 한곳만 쓰다가, 전혀 다른 서비스에도 데이터를 보내주어야 한다면 어떨까?

그러니깐 이런 말이다. 전자의 방법으로 진행하면 공유DB에서 각 서비스로 접근해서 데이터를 가져가야한다. 그런데, 추출로그 팀의 사업이 잘되어서 추출로그 db를 다른 외부의 소비자에게도 전달해주어야 한다고 보자. 이렇게 되면 공유 db 조직에서 추출로그 node 안에 agent 를 만들어서 외부로 발송해주거나, 공유db에서 추출로그 db의 내용만 추려내서 외부 소비자에게 발송을 해주어야 하는 모순이 생긴다. 맥락은 추출로그팀의 사업이 잘된건 추출로그팀에서 해결할 책임이 있다는 것이다. 전자의 방법으로 하면 추출로그 팀이 한창 열정적이고 의욕적일 때 발목이 잡히는 문제도 있고, 왜 추출로그 팀이 잘되었는데 공유팀에서 일을하냐는 불만도 자연스럽게 나온다. 시스템적으로도 사람 심리와 마찬가지이다. 왜냐면 그 시스템을 관리하고 책임을 져야할 것은 각 비지니스에 관심이 있는 비지니스 오너들이기 때문이다. 만약 회사의 정치나, 구조적인 문제로 공유팀에서 해줘야 한다면 공유팀에서 agent를 개발 해서 로그추출팀에 만들어주는 것처럼 해야한다.
