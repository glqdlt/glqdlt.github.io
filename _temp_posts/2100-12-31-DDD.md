...
-- 공개 호스트 서비스
  ...
  반부패 계층과 마찬가지로, 소비자가 어떻게 해석을 할것인가에 대한 이야기를 한다. 다만, 반부패 계층과 달리 공개 호스트 서비스는 id@google.com 에 대해 의존을 할 수 있는 셈이다. 해석하기 보다는 구글의 공개 호스트 API를 이용해서 해당 계정에 대한 식별 정보를 모두 호출해서 제공받을 수 있기 때문이다. 조금 더 우리 한국형 실무에 대해 생각하면, DB를 제공하는 것도 일련의 공개 호스트 서비스에 해당할 수도 있다. DB의 특정 테이블에 유저정보가 존재한다는 식으로 jdbc 로 알아서 읽어가라는 프로토콜을 "공개" 되어 있다면 이에 해당한다.
