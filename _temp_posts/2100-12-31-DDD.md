...
종류를 Builder 패턴으로 체이닝해서 공유커널&준수자 유형 이런식으로 현재 업무를 생각해보면  이해가 쉬울 것이다.
...
-- 공유 커널
  ...
  말 그대로 Model 에 해당하는 라이브러리를 서로 공유하는 개념이다. 공표된 언어와 유사한데, 차이점이라면 언어에 대한 의존도가 생긴다는 것이다. 
...
-- 반부패 계층
  ...
  일반적으로 많이 하는 형태이다. 이름이 부패라는 의미가 들어가서 그렇지, 가장 보편적인 방법이다. 공급자에게서 엔티티를 내려 받아 단순히 소비하는 형태를 의미한다. 예를 들면 facebook 연동, google 연동 등이 있다. 예를 들어서 openId라는 개념에서 인증을 한다면, 소비자는 google 이 내려주는 google 계정정보를 자신의 내부에서 관리하는 유저 ID로 변환하는(해석 또는 번역 또는 컨버팅) 아답팅 작업을 진행하는 걸 의미한다. 유저라는 개념을 google에서는 id@google.com 으로 하고, 이에 대해 로그인을 승인한 정보를 소비자에게 내려줬다면, 소비자 내부 바운디드 context에서는 이를 userNo = 199 번으로 번역해서 사용할 수도 있는 것이다.

-- 공개 호스트 서비스

  반부패 계층과 마찬가지로, 소비자가 어떻게 해석을 할것인가에 대한 이야기를 한다. 다만, 반부패 계층과 달리 공개 호스트 서비스는 id@google.com 에 대해 의존을 할 수 있는 셈이다. 해석하기 보다는 구글의 공개 호스트 API를 이용해서 해당 계정에 대한 식별 정보를 모두 호출해서 제공받을 수 있기 때문이다.

--- 공표된 언어
  
  gRPC 의 model 과 같은 개념이다. A와 B 컨텍스트 사이에는 공통으로 공유하는 공통 Model 을 가지게 된다.  공유 커널과 거의 유사하지만, 각기 다른 언어로 처리할수 있다는 점에서 공유 커널과의 차이점이 있다.
