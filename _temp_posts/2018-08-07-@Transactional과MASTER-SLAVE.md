...
처음에는 AOP 로 접근해서 해결해볼까 하는 생각을 했엇는 데, 손을 안 더럽히고 할 만한 구조가 발견 되지 않아 나의 얕은 지식으로는 단기간에 해결하기가 어렵다는 생각이 들었다.

결국은 언제나 그렇듯 이상은 이상이고 현실은 발 등에 불이 떨어지는 상황이다 보니, 서비스에 빠르게 적용되어야 했기 때문에 1. 유저가 가장 많이 접하는 로직 이거나 2. 무거운 스키마에 접근하는 로직 위주로 선별해서 단순하고 무식한 방법으로 작업을 진행하기로 했다. 

이 무식한 방법이라 함은 어플리케이션의 구조가 화면/서비스/영속성 tier(계층)이 구분되어 있는 구조에서나 할 수 있는 방법으로, 서비스 단에서 Master, Slave Repository 를 상황에 맞게 호출하게 하는 것이다. 다행스럽게도 소스가 나름 계층이 구분되어 있었기에 기존 Datasource 외에 Slave Datasource를 추가적으로 만들고, 서비스에서 호출 하던 기존 Master Repository(또는 DAO) 를 Slave 로 바라보도록 수정하는 식으로 처리 했다.
...
반영 된 소스는 몇 주동안 무리 없이 안정적으로 동작했다. 그도 그럴 것이 손만 피곤하지 크게 부담 스러운 작업을 한 것이 아니기 때문이다. 그렇다고 계속도록 비슷한 작업을 해야 할지도 모른다고 생각하면 끔찍하기에 다른 방법이 없을까 하는 생각을 계속 해왔다. 
...
이를 보며 틈틈히 공부하고 실험해보면서 최근에 진행한 이벤트를 마무리하고 숨 쉴 만한 시간이 생기면서 리팩토링 작업을 진행하게 되었다.
