...
객체지향 프로그래밍은 어떠한 로직이 진행되는 데에 있어서 객체의 상태를 변화시켜가며 작업하는 일련의 실행이 되어야 넘어가는 (상태가 변화되어야 하는) 패러다임이기에 기능은 항상 즉시실현되어야한다. 이래서 객체의 상태를 통해 일련의 커뮤니케이션이 되기 때문에 이름이 객체지향인것이다.

().().().()

반면 함수형 프로그래밍은 이름 그대로 객체가 먼저가 아닌, 함수가 먼저 나오는 것처럼 연산에 대해 집중하기 때문에 값의 상태가 어떻건 말건 연산 위주로 정의를 하는 데에 목적을 두고 이 연산이 언제 실행이 될 것인가? 에 대해 관점을 둔다.
