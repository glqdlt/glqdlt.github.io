...
어떠한 문제가 있는지 알겠는가? 초등학생이 봐도 어떠한 문제인지가 느껴진다.

아직도 모르겠다고? 그렇다면 여기에 아래와 같이 사고 가정을 해보자.

- 100번째 신규 게임 채널 채널100이 추가되었다.

멤버변수 channel100 이 추가되어야 하기 때문에, 서비스를 중단시키고 컴파일 후 패치를 해야한다.
...
집계수를 저장하는 시스템이나, 집계수를 화면에 뿌려주는 시스템이나 어느 곳에서 작업을 하더라도 100개의 채널을 직접 호출하는 식으로 접근이 된다.

여기에 자바의 경우 리플렉션 API를 사용해서 클래스에 속한 모든 필드를 순회하며 처리하는 식으로 하면 어떨까? 라는 아이디어를 낸 사람이 있다. 괜찮은 접근이지만 만약 channel 관련 필드 외에 다른 gameId 나 별도의 추가적인 필드가 생기면 이를 다 필터링하기 위한 하드코딩(식별) 작업이 들어간다.

결론적으로는 배열 구조를 담기 위한 모델링이 구성이 되어야 한다. 아래의 모델링으로 재설계가 필요하다.
...
게임의 경우 외부 인프라와 조인할 일이 많다. 대표적으로 채널링이나 퍼블리싱의 사용 등이 있다.
쉽게 말해 구글 로그인하기 나 카카오로 로그인 하기 와 같은 개념을 의미한다.

자 아래의 비지니스 정책이 있다고 가정을 하고, 이를 구현해본다고 해보자
