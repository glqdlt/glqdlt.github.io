...
예를 들어 특정 테이블에 아래와 같이 flag 컬럼이 일반적으로 0 or 1 이 아니라 Y 나 N 으로 되어있는 경우가 있다.
...
...
someRepo.findAllFlagTrue();
// select ... flag =  1;
```
와 같은 엔티티 필드와 FlagTrue 메소드를 사용할 수 없다.

왜냐면 findAllFlagTrue(); 시에 flag=1 이런식으로 변환되기 때문이다.

이를 해결하기 위해서는 [참조](https://stackoverflow.com/questions/50750028/jpa-findall-order-by-a-linked-list-size) 에 나오듯이

conventer 를 새로 정의 해줘야 하는 데, 좀더 쉬운 방법으로 하이버네이트에서 미리 정의 된 @type 이라는 어노테이션으로 처리할 수 있다.
...
[plantuml erd](https://github.com/plantuml/plantuml/pull/31) 가 나왔다고 함
