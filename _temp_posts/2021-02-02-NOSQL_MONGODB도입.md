...
몽고 DB는 신입 시절 때부터 익히 들었다, 당시에 MEAN (몽고DB, 엘라스틱서치, 앵귤러, NODEJS) 스택이라는 이름으로 거창하게 유행한 적이 있었는데 이게 무엇인지를 알수 없었다. 그냥 단순하게 이직하고 싶은  동경하는 대기업의 채용공고 우대사항에 적혀 있는 단어더라 이 정도로의 관심만 갖고 있었다.

지금은 과거처럼 유행보다는 NOSQL의 필요성을 느끼면서 왜 난 이것을 모르고 살았었냐며 시간을 돌리고 싶은 마음이다.

현재 재직 중인 곳에서의 웹서비스에는 영속성 계층을 RDB 기반으로만 처리를 하고 있다. 통합 관제를 위해 NOSQL 기반의 별도의 중앙 집중 로그 저장소가 별도로 비치되어 있지만, 특별한 목적을 위한 용도로 사용중이고, 실제 서비스에서는 NOSQL 을 사용하고 있지 않다. 

RDB 를 사용하는 것은 아래의 이점이 있기 때문에 사용중에 있다.

- 관계 구조를 가지는 정형화 데이터의 저장
- 복잡한 트랜잭션 지원
    - 로컬 DB기준으로 권한만 허락한다면 어떠한 스키마끼리도 트랜잭션을 걸 수 있고, 같은 벤더의 DB라면 원격 트랜잭션(DB 링크 등)이나 분산 트랜잭션(2PC 등)도 가능하다.

서비스 간의 복잡한 트랜잭션을 처리하기 위한 용도로 사용 되고 있다. 예를 들어서 게임 쿠폰을 사용하면 유저 재화 1000 게임머니가 지급된다는 비지니스 정책이 있다면, 이를 구현하기 위해 게임쿠폰 스키마와 유저 재화 스키마에 변경작업을 하고, 별도의 로그 테이블에도 유저 이벤트를 기록해야한다. 복잡한 트랜잭션이란 이러한 여러 엔티티들이 얽힌 트랜잭션 처리를 말한다.

최근에는 MSA 와 DDD 를 알게되면서 이는 한계가 명확한 낡은 방법이라는 것을 깨우치고 있다.

### RDB 에서의 상속이란

RDB는 자료 구조를 확장하는 데에 한계가 있다. 한계가 있다는 말에는 여러가지 기능과 용도가 포함되어 있는 데, 이 챕터에서는 구조화된 데이터만의 삽입을 얘기하려 한다. 자바 vs 자바스크립트 간의 객체 모델링의 유연함으로 생각해도 좋다.

예를 들어서 아래처럼 어떠한 상속 관계를 가지는 모델이 있다고 가정을 해보자.

![](images/a5a28f55.png)

RDB 에서는 위 상속 관계를 표현하기 위해서는 별도의 기능이 없으며, 아래의 2가지 패턴들 중에서 흉내를 내야한다.
...
RDB 에서 데이터라 하는 (우리가 행이라고 하는) 레코드들은 객체(클래스의 인스턴스)와 같다고 볼수있고, 레코드들은 테이블의 데이터 집합이니 테이블은 클래스에 빗대어 생각해볼수있다. 각 컬럼은 클래스의 속성(멤버변수 또는 필드)에 매핑해보면 얼추 맞아 떨어진다.

자바나 자바스크립트에서는 객체가 어떠한 타입의 자료형(클래스 또는 함수)인지를 판단하는 메타데이터가 객체의 숨겨진 곳에 내장하고 있기에 instance of 와 같은 키워드로 객체의 원형정보를 유추할수가 있다.

다만 RDB에서는 이러한 객체 원형 타입정보를 나타내는 데이터가 없기 때문에, 해당 레코드가 어떠한 자료형이 실체화된 인스턴스인지를 나타내는 데이터 필드(컬럼)이 필요하다. 이에 해당하는 것이 ```type``` 이라는 이름의 컬럼이다. 
...
(정규화기반은 손이 많이 가는 관계로, 편의상 싱글 테이블 기반으로 설명하겠다. 결과는 똑같다.)
...
결국 위 테이블을 모델링으로 표현해보면 아래와 같은 결과와 같다는 것을 알수있따.

![](images/9033c74c.png)

그럼 다른 방안은 없을까? 아래처럼 type 을 더 쪼개는 type 필드를 만들고 이는 type =='b' 인 경우에만 사용하는 식으로 한번 접근을 해보자.
이 경우 SubTypeB 인지 SubTypeC 인지에 대한 확장 타입을 구분하기 위해서 별도의 flag 를 나타내는 필드(컬럼)을 추가로 만들어야 한다.
...
결과를 보면 RDB의 한계 때문에 어플리케이션의 모델링이 자연스럽게 표현하지 못하고 영향을 받게 되는 점을 알 수가 있다.
...
참고로 엔터프라이즈 아키텍처 패턴에서는 이를 xxx 패턴이라고 한다 (지금 기억 안남, 마틴 파울러 책에 있었던 거로 기억)
