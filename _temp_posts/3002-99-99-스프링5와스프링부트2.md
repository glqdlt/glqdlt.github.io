...
스프링부트 앱이 시작될 떄의 이벤트를 감지해서 특정한 로직을 수행시키고 싶다면 CommandLine 인터페이스를 구현해서 처리할 수가 있다.
...
```@ConditionalOnClass``` : 이녀석은 특정 Class 가 클래스로더에 존재하면 이를 Bean 으로 등록시켜라이다, 유사한 것으로 ```@ConditionalOnBean ``` 가 있는 데, 이 친구는 Bean 이 검색되면 Bean 으로 등록시켜라 이다. 
