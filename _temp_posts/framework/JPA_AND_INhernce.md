...
팁이지만 위의 경우는 슈퍼테이블의 PK를 하위 테이블에도 PK 로 JOIN 하는 식별 관계 방법을 얘기하는 데, 굳이 저렇게 하지 않고 비식별관계(부모의 키를 자식에서 PK로 사용하지 않는 것) 으로 처리해도 무관하다. 이 경우 @PrimaryJoinColumn 어노테이션은 필요 없고, 그냥 일반적으로 외래키 매핑 하듯이 필드에 어노테이션 정의를 하면 된다. 단 이 경우, 부모의 getter/setter 를 호출시에 실제 반환 값으로 외래키로 매핑된 부모 타입의 필드의 데이터를 반환하게 해줘야 하는 데, 결국 부모의 모든 getter setter 를 오버라이딩 해야하기 떄문에.. 어떻게 보면 그냥 식별관계로 구성해서 PrimiaryJoinColumn 정의 하는 게 속편할 수도 있다 -_-;;
