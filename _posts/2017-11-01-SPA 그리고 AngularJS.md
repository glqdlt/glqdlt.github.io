...
## 요즘 Web Application : Single Page Application 
...
SPA(Single Page Application)는 문자 그대로 풀이해보면 '화면 전환이 없는 단 하나의 페이지(인스턴스)' 를 뜻 한다. 화면 전환이 없다는 말은 리다리엑션을 하지 않았거나, 서버에서 새로운 View를 제공 받지 않았다는 말이 된다. 마치 얘기만 들어서는 아무것도 하지 않는 대기 상태를 뜻하는 것 같다. 그런데 Angular나 ReactJS 로 만들어진 최신 트랜드의 웹 서비스 화면에서는 가만히 있어도 내가 보고 있던 화면이 갱신되거나, 알림이 Notification 되거나 한다. 과연 이것을 보고 아무것도 하지 않는 상태라고 말할 수 있을까?

SPA가 말하는 화면 전환이 없다는 말의 의미는 서비스 핸들링의 주체가 Server 가 아닌 Client에 있다는 의미로 받아들이면 된다. 위에서 잠시 이야기 했지만 SPA가 갖는 진정한 의미는 Single Page가 아닌 App으로서 존재한다는 의미가 강하다. 

어떤 사람은 SPA를 User 인터렉션 (사용자 반응)이 강한 다이나믹한 UI(Page) 라고 생각할 수도 있다. 틀린 말은 아니다, 하나의 Application과 다를 것 없이 User의 Event를 처리할 수 있는 막강한 기능들을 가지고 있기 때문이다. 하지만 이 말은 단순하게 일부분만을 바라본 이야기이다. 
기존의 전통적인 Web Page 환경에서도 Javascript 를 통한 여러가지 이벤트 처리 및 애니메이션, 비동기 데이터 갱신 등을 할 수 있었기 떄문이다.


## F / B 포지셔닝의 명확한 포지셔닝

최근에 감명 깊게 읽었던 책으로 [클라이언트 서버 웹 앱 만들기. 저, 캐지미어 새터노스]() , 모든 맥락에서 '클라이언트 // 서버' 라고 명확히 단절해서 이라고 표현한다. 완전 별개의 단절 된 형태로 이야기하는 것은 아니지만, 포지셔닝에 대해서 명확히 선을 긋고 있다. 아직도 과거의 전통적인 웹 페이지, 모노로틱 웹 서비스 아키텍처로 밥을 먹고 살고 있는 이들에게는 이게 무슨 뚱딴지 같은 소리일까 싶을 것 같다.

이 말은 프론트/벡엔드가 명확히 나뉘어 지는 워크 아키텍처 떄문일 수도 있지만, 기능상으로만 봐도 SPA 는 단순 Document 가 아닌 Application 이라고 보는 것이 옳다, MVC를 스트럭처를 가진 SPA를 어찌 단순 Web Page로 볼 수 있단 말인가? 즉 Html로 접근 할 것이 아닌 하나의 Application 으로 접근해야 한다.
