...
그도 그럴 것이 필자가 근무하고 있는 곳은 수 서버 개발팀으로 많은 데이터의 핸들링과 유저 커넥션을 처리하는 '데이터 딜리버리' 서비스를 구현 하는 곳에서 근무하고 있다. 
...
그래도 굳이 나를 지목해서까지 세미나를 준비해달라고 하는 것은 어떠한 분위기 쇄신을 위한 요청일 것이란 생각이 들었다. 가까이서 구언으로 요청을 했다면 물어라도 봤겠지만 저 먼 벨기에라는 외국에서 보낸 메일이었기에.. 시차 상황도 있어서 팀장님의 거룩한 뜻을 깊게 생각하지는 않고 의문을 가진 채 세미나 준비를 해보았다.

일단 방금도 얘기 했듯이 분식집의 주방 이모들님들에게 일본식 라멘 제조법을 설명해야 하는 상황이었다. 사실 팀원 분들은 국내에서도 알아주는 내놓으라는 엔지니어의 집단이다, 이런 분들에게 주방 이모님들과 비유한다는 것 자체가 실례일 수 있다. 팀 동료들도 Angular 든 ReactJS든 이름은 많이 들어봤을 텐데, 굳이 필요성에 대해서 인지를 못하기에 선듯 관심을 가지진 못했을 수도 있다. 나 역시도 누구라도 알법한  웹 서비스의 엔지니어가 되보고자 노력했던 시간이 없더라면 관심도 가지질 않았을 것이다.

 이유야 어쨋건 결국 분식 라면과 일본식 라멘의 차이점을 설명해야 하는 상황은 변하지가 않기 때문에..여가시간에 개인적으로 갖고 놀던 Angular의 지식 보다(아마 트랜드에 가장 민감해하고 팀에 반영하고자 하던 성격 때문에 팀장님이 나를 지명했으리라는 생각이 확고하다)는 SPA 어플리케이션에 대해서 설명하는 것이 더 중요하단 생각이 들었다.
