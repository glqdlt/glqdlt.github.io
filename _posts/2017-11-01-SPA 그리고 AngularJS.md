...
사실 예제라고 비유하기 위해 말한 것이지만 국내에서 나름 알아주는 서버 엔지니어링이 모인 팀을 분식집에 비유한다는 것은 어찌 보면 실례를 겸한 걸 지도 모른다. 어찌 보면 팀 동료들도 Angular 든 ReactJS든 이름은 많이 들어봤을 텐데, 굳이 필요성에 대해서 인지를 못하기에 선듯 관심을 가지진 못했을 수도 있고..
나 역시도 유명 단순히 트랜드에 관심이 많은 성격 탓에 Angular에 관심을 가졌을 뿐이지, 업무 환경을 이유로 둔다면 딱히 관심을 가질 껀덕지는 없었을 것이다. 단순히 예시로만 말하는 것이니 큰 의미는 두지 말았으면 한다.
