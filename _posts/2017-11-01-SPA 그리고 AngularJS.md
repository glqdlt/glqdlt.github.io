...
요청을 받자 마자 느낀 감정은 일단 나를 지목하신 것은 아무래도 팀 내에서 트랜드에 가장 관심 가지던 성격 때문에 팀장님이 나를 지명했으리라는 생각이 확고했는 데, 이것보다 문제는 왜 세미나 준비야..? 란 것보다는 왜? Angular를 ? 란 주제 선정에 대해 느낀 의무점이 가장 컸다. 

그도 그럴 것이 필자가 근무하고 있는 곳은 서버 엔지니어 팀으로 많은 데이터의 핸들링과 유저 커넥션을 처리하는 '데이터 딜리버리' 서비스를 구현 하는 팀에서 근무하고 있다. 웹 서비스를 구현하거나 운영하는 팀과는 거리가 조금 멀다.

하는 일에 대해서 조금 더 자세히 얘기하면 외부의 데이터나 내부의 데이터를 취합하거나 변경하는 biz service를 만들고 Restful Api를 통해 내/외부에 제공하는.. 요즘 주변의 흔하게 볼 수 있는 서버 사이드(Server Side) 개발을 많이 하고 있다. 

나에게 있어 (우리 팀에 있어) Web Application은 서버 운영에 필요한 간단한 Admin Console Tool 정도로 활용하고 있다. 

Application은 간단한 CRUD 형태의 게시판이나 Config의 0 과 1 을 제어하는 로직이 대부분이다. 이런 팀 환경에서 갑작스레 Angular를 들고 세미나를 하겠다고 하는 것은..

 마치 분식집에서 일본식 돈코츠 라멘을 주문하는 것과 같다는 생각이다. 뭐, 사실 이게 불가능한 이야기는 아니다.. 엉뚱한 주문으로 볼수 있지만 만약 분식집의 주방 이모가 만들 수 있는 재료와 실력이 있다면 만들어서 식탁에 내놓기야 할 수 있을터이니 말이다. 즉 팀장님의 세미나 요청은 위의 일본 라면의 예처럼 굉장히 아이러니(?)한 주문인 셈이다.


사실 팀원 분들을 분식 집의 주방 이모님들과 비유한다는 것 자체가 실례를 범한 걸 지도 모른다. 우리 팀은 국내에서 내놓으라는 서버 엔지니어링 팀이고, 외부에서도 널리 알려진 곳이다. 어찌 보면 팀 동료들도 Angular 든 ReactJS든 이름은 많이 들어봤을 텐데, 굳이 필요성에 대해서 인지를 못하기에 선듯 관심을 가지진 못했을 수도 있고..

나 역시도 누구라도 알법한 유명 웹 서비스 회사의 엔지니어가 되보고자 노력했던 시간이 없더라면 Angular에 관심도 가지질 않았을 것이다. 단순히 예시로만 말하는 것이니 큰 의미는 두지 말았으면 한다.

잡설이 많았는 데, 어찌 되었건 팀장님의 이런 요청은 내의 분위기 전환을 위한 요청일 일지도 모른다. 평소처럼 바로 옆자리에서 구언으로 일을 요청하셨더라면 질문이라도 해봤겠지만 저 먼 벨기에 에서 메일로 요청하신 일이었기에 의문의 해답 보다는 우선 세미나 준비를 하기로 했다.

일단 Angular라는 주제는 방금도 얘기 했듯이 분식집의 주방 이모들님들에게 일본식 라멘 제조법을 설명해야 하는 상황 처럼 팀과는 어울리지 않는 주제라고 생각 한다. 그래서 개인적으로 여가 시간에 갖고 놀던 Angular 프레임워크의 Architecture에 대해 풀어 가는 것 보다는 SPA 어플리케이션에 대해서 설명하는 것이 더 중요하단 생각이 들었다. 왜 일본식 라면 집이 있는 지, 어떨 때 만드는 것인지, 맛의 차이가 어떠한지를 알리는 것이 더 중요하기 때문이다.
...
사람에 따라서는 이 말이 다소 황당하게 느껴 질 수도 있다고 생각한다. 만약 Web의 초창기에 시간이 머물러 있는 사람이라면 Web은 문서를 공유하기 위한 플랫폼이라고 괴성을 지를 지도 모르겠다. 

SPA(Single Page Application)는 문자 그대로 풀이해보면 '화면 전환이 없는 단 하나의 페이지(인스턴스)' 를 뜻 한다. 화면 전환이 없다는 말은 리다리엑션을 하지 않았거나, 서버에서 새로운 View를 제공 받지 않았다는 말이 된다. 이야기만 들어서는 아무것도 하지 않는 대기 상태를 뜻하는 것 같다. 그런데 Angular나 ReactJS 로 만들어진 최신 트랜드의 웹 서비스 화면에서는 가만히 있어도 내가 보고 있던 화면의 데이터가 갱신되거나, 메시지 알림 팝업이 Notification 되거나 한다. 과연 이것을 보고 아무것도 하지 않는 상태라고 말할 수 있을까?

SPA가 말하는 화면 전환이 없다는 말의 의미는 웹 서비스 이벤트의 주체가 Server 가 아닌 Client에 있다는 의미로 받아들이면 된다. 위에서 잠시 이야기 했지만 SPA가 갖는 진정한 의미는 Single Page가 아닌 App으로서 존재한다는 의미가 강하다. Application 으로서의 다양한 기능을 가지고 있기 떄문에 이러한 것들이 가능한 것이다.

이런 시각적인 이벤트 부분 떄문에 어떤 사람은 SPA를 User 인터렉션 (사용자 반응)이 강한 다이나믹한 UI(Page) 라고 생각할 수도 있다. 틀린 말은 아니다, 하나의 Application과 다를 것 없이 User의 Event를 처리할 수 있는 막강한 기능들을 가지고 있기 때문이다. 하지만 이 말은 단순하게 일부분만을 바라본 이야기이다. 
기존의 전통적인 Web Page 환경에서도 Javascript 를 통한 여러가지 이벤트 처리 및 애니메이션, 비동기 데이터 갱신 등을 할 수 있었기 떄문이다.
