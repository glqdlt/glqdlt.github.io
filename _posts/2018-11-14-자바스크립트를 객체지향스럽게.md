...
이 아티클은 [객체지향 자바스크립트 3/e](http://www.yes24.com/24/goods/51335821) 를 읽으면서 자바스크립트 기초를 복습한 요약본입니다.
...
// Array.from(arr) 은 인자로 들어오는 배열을 deepCopy 한 배열을 반환해준다.
const someArr = [1,2,3,'test'];

const arr2 = Array.from(someArr);
console.log(`${arr2}`);
