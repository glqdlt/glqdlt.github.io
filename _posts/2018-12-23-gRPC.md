...
gRPC 는 Google 에서 만든 RPC(Remote Procedure Call)에 속하는 프레임워크 입니다.
...
농단 반 섞인 것 이지만, Restful API 대비 효율이 3배 좋다고 합니다. 자세한 것은 [gRPC vs REST slideshare](https://www.slideshare.net/borisovalex/grpc-vs-rest-let-the-battle-begin-81800634) 에서 참조.
...
- Java
- Android Java
- Node.js (프론트엔드는 공식적으로는 아직 미지원이다, [써드파티](https://github.com/improbable-eng/grpc-web)가 있기는 하다.)
- Objective-C
- PHP
- Python
- Ruby
...
최근에는 Thrift 가 소셜 네트워킹에서 많이 사랑 받으면서 점차 RPC의 관심도가 커지고 있는 추세입니다.
...
당연한 이야기이지만, 자바로 먹고 살고 있기에 예제 소스는 자바 기반입니다. (추후 NodeJs 기반 Client 정도 작성 할지도 모르겠네요. -_-;)

모든 소스는 [개인 github](https://github.com/glqdlt/ex-grpc) 에서 확인할 수 있습니다.
...
.proto 는 proto buffer 의 syntax 로 작성됩니다. (Dockerfile 을 다뤄본 사람이라면 익숙한 구조.)
