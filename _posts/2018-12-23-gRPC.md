...
기존 http 1.1에서는 요청과 응답이 있을때에만 연결을 하고 끝나면 다시 연결을 끊었다면, http2 에서는 hPack 에서 지속적으로 연결을 맺고 있고, 메세지(요청과 응답)가 필요할 때에만 리소스를 사용합니다. http2의 지속적인 연결은 RST_STREAM 프레임을 보내서 연결을 해지할 수 있습니다.
