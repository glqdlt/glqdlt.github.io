...
http2 에서는 http1.1 과 달리 패킷 헤더 압축을 위해 hpack에서 기본적으로 클라이언트와 서버 간의 지속적인 옅은 state를 가지게 됩니다. (이게 싫어서 만들어진 것이 [CASHPACK](https://github.com/Dridi/cashpack)] 이라는 것이 있습니다.) 다만 기존 http1.1과 유사하게 메세지(요청과 응답)이 필요할 때마다 단일 연결 내에서 새로운 스트림을 열어서 통신을 하는 구조를 가지고 있습니다.(메세지는 http1.1 에서는 1개의 요청과 1개의 응답이었던 반면, http2 에서는 N개의 요청과 N개의 응답을 보낼 수 있습니다.) 기존 http 1.1에서는 요청과 응답이 있을때에만 연결을 하고 끝나면 다시 연결을 끊습니다. 하지만, http2 에서는 hPack 에서 지속적으로 연결을 맺고 있으며, 메세지(요청과 응답)가 필요할 때에만 리소스를 사용합니다. 이 지속적인 연결은 RST_STREAM 프레임을 보내서 연결을 해지할 수 있습니다.
