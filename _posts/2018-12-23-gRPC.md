...
gRPC는 http 2 기반이기에 http2 를 포함하는 서블릿 컨테이너 구현체 중 하나인 tomcat 최신 버전에서 구현할 수 있지 않을까 생각을 했었습니다. 하지만 gRPC 에서 서블릿은 지원 대상이 아니라는 것을 알게 되었습니다. (netty 프레임워크 나 okHttp 는 지원)
