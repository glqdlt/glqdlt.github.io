...
gRPC는 Google 에서 만든 [RPC(Remote Procedure Call)](https://ko.wikipedia.org/wiki/%EC%9B%90%EA%B2%A9_%ED%94%84%EB%A1%9C%EC%8B%9C%EC%A0%80_%ED%98%B8%EC%B6%9C) 프레임워크 입니다. gRPC는 이름만 봐도 유추되듯 단순하게 google + RPC 란 의미를 가지고 있습니다.
...
- Application Layer
    ...
    실제 우리 개발자들이 신경써야 할 영역 입니다. proto 파일을 정의하고 정의 되어서 컴파일 된 구현체들을 실제 어플리케이션 로직에 연결하는 작업까지를 담당합니다.
...
enum 과 같은 열거형 상수 타입도 지원하며, repeated 키워드를 통해 배열도 선언할 수 있습니다. 프로토콜 버퍼는 기본적으로 [빌더 패턴](https://ko.wikipedia.org/wiki/%EB%B9%8C%EB%8D%94_%ED%8C%A8%ED%84%B4)을 사용하며, required 와 optional 과 같은 객체 초기화에 꼭 필요한 필드를 강제화 하거나 선택적 옵션 필드로 선언할 수 있습니다.

또한 프로토콜버퍼에서는 하위 호완성을 위해 메세지 필드의 순서를 정의합니다. 이 특별한 정수형 순서는 바이너리 데이터에서 어느 위치에서 데이터를 읽어야 하는지의 알려주는 정보로 활용 됩니다. 

예를 들어  클라이언트가 4번까지만 정의 된 기존 버전의 메세지 정의를 사용하고 있고, 서버에서는 더 상세하고 5번,6번이 추가 된 메세지를 사용 하더라도 클라이언트, 서버에서 서로 호환성에 문제 없이 사용할 수 있습니다. 

이는 분산 환경에서 항상 동일한 최신 버전으로 계속적으로 갱신하기에 부담스러운 부분을 해소해주는 좋은 기능입니다. 
이를 통해 개별적으로 클라이언트와 서버는 자연스럽게 느슨한 결합을 유지할 수 있게 됩니다. (proto 가 업데이트 되었다고 해서 클라이언트가 필드 사용 유무에 따라 강제 업데이트를 하지 않아도 됨) 

단, 여기에도 제약은 있는 것으로 메세지의 순서가 보장되어야 하며 필드(또는 프로퍼티) 네임은 항상 동일해야 하는 점이 있습니다.

위의 기본적인 기능 외에 gRPC 에서의 프로토콜 버퍼에는 추가적으로 아래와 같이 서비스 인터페이스를 정의할 수 있습니다.
...
기존 http 1.1에서는 요청과 응답이 있을때에만 연결을 하고 연결을 끊는 반면, http2 에서는 hPack 에서 지속적으로 연결을 맺고 있고, 메세지(요청과 응답)가 필요할 때에만 리소스를 사용합니다. http2 스트림은 정상적인 송수신이 완료되었을 때 최종으로 발생하는 END_STREAM 이 도착해서 종료(CLOSEED) 상태에 들어섭니다. 하지만 의도하여 연결 도중에 즉각 스트림을 해지하고 싶을 때에는 누구든지 RST_STREAM 프레임을 보내서 연결을 즉시 해지할 수 있습니다.
...
자세한 것은 https://grpc.io/docs/guides/concepts.html#service-definition 문서를 참고할 수 있으며, 
...
.proto 는 proto buffer 의 문법으로 작성됩니다. 일반적인 자바의 class 문법과 비슷합니다. 
...
// message 키워드가 붙는 것은 실제 메세지에 실릴 Model 객체입니다. 재밌는 점은 필드의 순서를 정의할 수 있고 빌드 패턴으로 생성됩니다.
message UserRequest {
    string id = 1;
}
...
저는 주로 프로젝트 도구로 Maven 을 사용합니다, Maven 에는 protobug-maven-plugin 이 있습니다.
...
Client 에서는 stub 이라는 개념이 등장 합니다. 이 stub 은 '어떠한 것이 존재할 것이다' 라고 추측하는 의미를 갖습니다. (JAVA RMI(Remote Method Invokation) 에서는 stub / Skeleton 이라 불리웁니다.) stub 은 서버에 사전에 약속한(proto 로 정의한) 메소드가 존재할 것이라 가정하는 클라이언트의 호출 인터페이스입니다.
...
gRPC 에서는 있을 것이다라고 추측하는 반면, TDD 에서는 확신하는 개념으로 조금 차이가 있습니다.
