...
기존 http 1.1에서는 요청과 응답이 있을때에만 연결을 하고 연결을 끊는 반면, http2 에서는 hPack 에서 지속적으로 연결을 맺고 있고, 메세지(요청과 응답)가 필요할 때에만 리소스를 사용합니다. 스트림의 연결은 정상적인 송수신이 완료되었을 때 최종으로 발생하는 END_STREAM 이 도착해서 종료(CLOSEED) 상태에 들어섭니다. 하지만 의도하여 연결 도중에 즉각 스트림을 해지하고 싶을 때에는 누구든지 RST_STREAM 프레임을 보내서 연결을 즉시 해지할 수 있습니다.
...
자바에서 gRPC는 총 3가지의 stub을 제공합니다.(언어마다 차이가 있습니다.) 기본적으로 동기/비동기로 구분되어 있으며, 동기 객체인 BlockingStub 을 제공하는 ```Grpc.newBrockingStub()``` 과 비동기 Stub을 다루는 ```Grpc.newStub()```이 있습니다. 또한 Java 의 Futre를 return 하는 ```Grpc.newFutureStub()```도 있습니다. 
