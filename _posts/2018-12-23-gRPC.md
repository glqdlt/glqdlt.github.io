...
gRPC가 만들어진 계기는 기존 google 에서 10년 이상 Micro Service 에서 stubby 라는 RPC를 사용해서 google 인프라에 정착시키고 사용되어져 왔습니다.

다만 stubby 는 특정한 표준화가 없었던 탓에 google 인프라에는 잘 정착되었지만, 범용적이지 못한 부분이 있었습니다. http2가 세상에 알려지면서, stubby를 표준화 할 수 있겠다는 영감을 얻은 Google은 Restful API 보다 효율이 더 좋은 RPC 를 쉽게 사용하게 하자는 취지와 함께 stubby 를 재작업하였고 그 결과가 gRPC 입니다. (자세한 개발 비하인드는 [gRPC blog](https://grpc.io/blog/principles)에서 원본을 찾아볼 수 있습니다.)
...
서버 푸시는 클라이언트의 초기 불러와야할 http resources (예: css, js 등) 들의 목록을 요청 받았을 때에, 클라이언트가 추가 요청하기 전에 사전으로 보내주는 기능입니다. ([http server push wiki](https://en.wikipedia.org/wiki/HTTP/2_Server_Push))
