...
gRPC 는 http2 를 기반으로 사용하는 RPC 프레임워크입니다. gRPC 를 사용한다는 것은 http2 에 대해서도 알아간다는 것을 의미하며, http2의 이해가 없으면 gRPC를 이해하기 어렵습니다. 때문에 이 아티클에서는 gRPC 외에도 http2 에 대한 이야기도 짤막하게 다루어봅니다.
...
gRPC 는 Google 에서 만든 RPC(Remote Procedure Call) 프레임워크 입니다. gRPC 란 이름을 보면 알겠지만, 단순하게 google + RPC 란 의미를 가지고 있습니다. -_-;

Micro Service Architecture가 보편화 되어 서비스 node 간의 통신 성능이 화두 되면서 자주 사용 되던 Restful API 보다 효율이 더 좋은 RPC 를 쉽게 사용하자는 취지로 만들어졌습니다. 

농단 반 섞인 이야기이지만 Restful API 대비 효율이 3배 좋다고 하며, 로드 밸런싱, 추적, 상태 확인 및 인증을위한 플러그 가능한 지원을 통해 데이터 센터에서 또는 서비스 전반에 효율적으로 연결할 수 있습니다. 자세한 이야기는 [gRpc 성능 차트](https://cloud.google.com/blog/products/gcp/announcing-grpc-alpha-for-google-cloud-pubsub) 에서 확인해 볼 수 있습니다.
...
RPC(Remote Procedure Call)는 의미 그대로 원격지에서 client(consumer) 가 server(provider) 의 기능을 직접 호출하고 그 결과를 받는 식의 자연스러운 로직 flow를 가지자는 개념입니다.
...
gRPC 의 큰 특징으로는 [protocol buffers](https://developers.google.com/protocol-buffers/docs/overview)의 사용으로 특정 언어 종속 없이 폴리그랏(polyglot) 환경을 꾸릴 수 있다는 점과 http/2 프로토콜 기반의 양방형 스트리밍을 할 수 있다는 점이 있습니다.

아래는 [official gRPC about](https://grpc.io/about/)에 실린 gRPC의 특징입니다.
...
gRPC를 통한 개발을 진행하면 가장 첫번째로 마주치는 것이 프로토콜 버퍼를 정의하는 데에서 부터 시작합니다. 정의 된 proto 파일은 프로토콜 버퍼 컴파일러를 통해 메세징 모델과 base 가 되는 서비스 코드(gRPC 에서만)로 컴파일 됩니다.

<img src="/images/grpc/pb.PNG"/>

프로토콜 버퍼는 폴리그랏 프로그래밍에 적절한 프레임워크입니다. proto 파일을 정의하여 언어 불문하고 다양한 언어로 컴파일 할 수 있습니다. (여기서 폴리그랏(polyglot) 이란 실 세계에서 언어에 관해 다각화란 의미를 가집니다. 여기에 프로그래밍이 붙여졌으니, 단일 언어의 한계를 극복하기 위한 [여러 언어로 코딩하는 프로그래밍](https://searchsoftwarequality.techtarget.com/definition/polyglot-programming)이란 의미를 가집니다. )

프로토콜 버퍼는 아래의 목록에 보이듯 현재 2019년 1월 기준으로 약 10개의 언어를 지원하고 있습니다. 
