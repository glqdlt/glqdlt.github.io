...
gRPC 는 Http 2 기반이기에, 서블릿4 구현체인 tomcat 9 버전으로 쉽게 동작할 수 있지 않을까 생각했다. 그러나 이에 대한 결과는 지원이 불가능하다. http2 를 구현한 구현체로는 spring 으로 치면 asynctemplate 의 구현체 범주에 속하는 경량 프레임워크 netty 나 okhttp 라이브러리 등과 같은 써드 파티들만 지원했다. 왜 그럴까?
