...
proto 문법에서 주의할 점은 필드의 순서를 정의하는 부분을 둘 수 있습니다. 이는 프로토콜버퍼에서 하위 호완성을 염두한 syntax 입니다. 이 특별한 정수형 순서는 바이너리 데이터에서 어느 위치에서 데이터를 읽어야 하는지의 알려주는 정보로 활용 됩니다. 

예를 들어  클라이언트가 8번까지만 정의 된 기존 버전의 메세지 정의를 사용하고 있고, 서버에서는 더 상세하고 9번,10번까지 추가 된 메세지를 사용 하더라도 클라이언트, 서버에서 8번까지만 사용한다면 호환성에 문제 없이 사용할 수 있습니다. 

이는 분산 환경에서 항상 동일한 최신 버전으로 계속적으로 갱신하기에 부담스러운 부분을 해소해주는 좋은 기능입니다. 이를 통해 개별적으로 클라이언트와 서버는 자연스럽게 느슨한 결합을 유지할 수 있게 됩니다. (proto 가 업데이트 되었다고 해서 클라이언트가 필드 사용 유무에 따라 강제 업데이트를 하지 않아도 됨) 단, 여기에도 제약은 있는 것으로 메세지의 순서가 보장되어야 하며 필드(또는 프로퍼티) 네임은 항상 동일해야 합니다.
