...
hpack 자체도 stateless 하게 사용하고 싶은 몇몇 사람들이 만든 cashpack 이라는 것도 있습니다.
...
http2 의 서버 푸시 기능 덕분에 나를 포함한 많은 이들이 웹소켓과 비교를 하곤 하는 데.. 전적으로 잘못된 생각입니다.
http2 의 서버 푸시 기능은 원칙적으로 static assets (리소스) 들을 클라이언트에 효율적으로 보내주기 위해 push 를 하는 개념입니다.
웹소켓처럼 요청 없이 응답 메세지(data)를 push 할 수 있는 것은 아니기에 두 개를 비교한다는 자체가 잘못 된 것입니다.
