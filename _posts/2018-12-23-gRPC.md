...
gRPC 는 Google 에서 만든 RPC(Remote Procedure Call)의 범주에 해당 하는 프레임워크 입니다.

클라우드 시대에 접어들어 MSA(Micro Service Architecture)가 보편화되면서, 기존 파편화 된 서비스 node 들 간의 restful api 통신을 효율이 더 좋은 RPC 를 쉽게 사용할 수 있게 하고자 만들어졌습니다. (농단 반 진담 반으로 Restful API 대비 효율이 3배 좋다고 합니다. 호달달 [gRPC vs REST](https://www.slideshare.net/borisovalex/grpc-vs-rest-let-the-battle-begin-81800634))

<img src="https://grpc.io/img/landing-2.svg"/>

(사족으로 MSA 에서 node 들 간의 통신에서 restful api 외에도 메세지 솔루션(예: apache kafka)들과도 비교할 수 있지 않은가 하는 생각을 해봤습니다. 좀 더 생각을 해보니 메세지 솔루션은 pub/sub 개념이고 원하는 시점에 호출하는 RPC 와는 설계와 개념에서 차이가 나겠단 생각이 들었습니다. ([gRPC vs Kafka](https://www.quora.com/How-should-I-choose-between-gRPC-and-Kafka-when-building-microservices)))
...
지원하는 언어는 [이곳에서](https://grpc.io/docs/quickstart/)서 참고할 수 있습니다.

- C++
- C#
- [Dart](https://www.dartlang.org/) (Google 에서 만든 Typescript라 보면 됨 -_-a.  JVM 기반 Transpile Language.)
- Go
- Java
- Android Java
- Node.js
...
gRPC에 대해 더 자세한 것은 [Official Concept](https://grpc.io/docs/guides/index.html) 에서 확인이 가능합니다.

## RPC ?

RPC(Remote Procedure Call)은 말 그대로 원격지에서 guest 가 host 의 function(method)를 호출하고 그 결과를 받아 같은 소스상에서 자연스럽게 흘러가는 flow 를 가지자는 개념입니다.

RPC 는 매우 오래 전 부터 있던 개념으로 (개인적으로 처음 접한 RPC는 신입 시절 [Adobe Flex ~ Java SOAP](https://www.tutorialspoint.com/flex/flex_rpc_services.htm)를 경험해본게 최초였음 -_-a) 매우 획기적인 컨셉에 비해 사용성이 어려워서 많이 알려지지 않았습니다.

원격지에서 호출한다는 개념 자체가 네트워킹으로 이루어진다는 것이고, Guest 와 Host 가 동일 선상에서 협약 된 구조를 가져야 한다는 전제 떄문에 사용성과 관리가 어려웠기 때문입니다.
...
최근에는 Thrift 가 소셜 네트워킹 기업들에서 사랑 받으면서 점차 RPC의 관심도가 커지고 있는 추세입니다.
...
gRPC 는 총 3계층 구조로 되어있습니다.

Application Layer 에서는 .proto 파일을 정의해서 Proto Buffer를 통해 구현체들을 generated 하는 역활이며, Application Layer 에서 generated 된 산출물들은(자바의 경우 .class) gRPC Core 를 통해 Http 2 위에 올라가게 됩니다. 

## gRPC with tomcat?

gRPC 는 Http 2 기반이기에, http2 를 포함하는 Servlet4 구현체 중 하나인 tomcat 9 버전에서 쉽게 동작할 수 있지 않을까 생각을 했봤는 데, 서블릿 구현체는 지원하지 않고  netty 나 okhttp 라이브러리 등과 같은 써드 파티들만 지원한다고 합니다. 

이유가 궁금해서 구글링으로 [관련 토픽](https://github.com/grpc/grpc-java/issues/1621)에서 여러 이유를 알 수 있었는 데, 
 gRPC 는 http2의 모든 소켓을 완전히 제어할 수 있어야 한다고 합니다. 서블릿은 이와 맞지 않는 설계 구조를 가지고 있기에 지원이 불가능했던 것으로 생각됩니다. (이유 중에서 컨테이너 모든 서블릿 인스턴스 들도 제어를 해야하는 얘기가 있는 것으로 보아 생각보다 큰 문제인 것 같음.) 그래도 영 불가능한 것은 아닌지, 몇몇 스레드를 보면 jboss 의 오픈소스 버전인 wildfly 를 기반으로 시도한 흥미로운 사례도 있어 향후 어떻게 발전할지는 모르겠습니다. -_-a

정리를 하면 서블릿 컨테이너의 http2 통신을 이용해서 gRPC 를 동작시키는 것은 불가능하고, 서블릿 컨테이너 안에서 별도의 소켓을 추가로 여는 식으로 (이에 대한 기능 구현체는 netty 나 okhttp) 진행한다는 것을 의미합니다.
...
간단한 예제를 통해 gRPC 를 살짝 맛 봤습니다.
당연하겠지만 자바로 먹고 살고 있기에 예제 소스는 자바 기반입니다. (추후 NodeJs 기반 Client 정도 작성 할지도 -_-;)

설명하는 모든 소스는 [github repo](https://github.com/glqdlt/ex-grpc) 에서 확인할 수 있습니다.

아래 부터의 설명은 편하게 음/슴체로 진행하겠습니다. 
...
[예제 소스](https://github.com/glqdlt/ex-grpc) 의 구조는 3개의 모듈로 이루어진 멀티 모듈 구조로 만듬.
...
model module source 에는 단순히 proto buffer 에 대한 정의를 하는 모듈이기에, 어떻게 보면 핵심이라 할 수 있음.

이 모듈에는 proto 의 정의와 이를 build 하기 위한 maven plugin 가 정의되어 있음.
...
proto 파일에서 generated 된 클래스를 base로 한 UserService 구현체
```java
...
[wiki](https://en.wikipedia.org/wiki/Stub_(distributed_computing)) 에서 자세한 내용을 알아볼 수 있음
...
여기서 주의 깊게 볼 것은 gRPC 에서 remote method 의 응답값을 가져올 수 있는 방법으로 총 3가지가 있는 데, 이 중에서 async 방법인 futureStub 을 사용했다는 점을 확인.
