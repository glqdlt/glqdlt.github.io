...
대표적으로 기존 요청(reqeust)과 응답(response)을 확장판 '메세지(message)' 개념이 있고, 통신 단위인 '프레임(frame)'과 실제 메세징이 올라가는 '스트림(stream)'이 있습니다. 쉽게 기차를 예로 들면, 프레임은 기차의 열칸(1호, 2호)과 유사하며, 스트림은 철도에 해당합니다. 
