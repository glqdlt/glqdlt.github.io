...
순서로는 gRPC 에 대해 가볍게 개념을 알아보는 시간을 가지고, java 기반의 간단한 코드를 실습해보는 것을 목표로 합니다. gRPC는 http2 를 사용하기 때문에 http2의 이해가 없으면 이해하기가 참 어렵습니다, 이런 이유로 http2 에 대해서도 간략히 짚어봅니다.
...
gRPC 는 Google 에서 만든 RPC(Remote Procedure Call) 기반 프레임워크 입니다. 클라우드와 MSA(Micro Service Architecture)가 보편화되고 서비스 node 간의 통신 성능이 화두 되면서 자주 사용 되는 Restful API 보다 효율이 더 좋은 RPC 를 쉽게 사용하자는 취지로 만들어졌습니다. 농단 반 섞인 이야기로 Restful API 대비 효율이 3배 좋다고 합니다. 이에 대한 이야기는 [gRpc 성능 차트](https://cloud.google.com/blog/products/gcp/announcing-grpc-alpha-for-google-cloud-pubsub) 에서 확인해 볼 수 있습니다.
...
gRPC 의 가장 큰 매력으로는 [protocol buffers](https://developers.google.com/protocol-buffers/docs/overview)을 통해 폴리그랏 언어 환경을 꾸릴 수 있으며,  http/2 프로토콜 기반의 양방형 스트리밍을 할 수 있다는 점이 있습니다.
...
http2 는 stateful 한가에 대한 질문은 예니오 (예 + 아니오) 입니다. 질문 자체가 조금 난해한 이유도 있지만, stateful 의 의미에 대해 먼저 정의 해야 이야기가 진행될 수 있습니다. 그럼 stateful 에 대한 대상을 connection 으로 바라본다고 정의를 하고 얘기를 해볼까 합니다. http2 의 connection 은 지속적(stateful) 한가요? 란 질문을 한다면 예라고 대답할 것 입니다. http2의 몇몇 기능 중 헤더압축은 stateful 하게 커넥션을 계속 맺고 있기 때문입니다.
