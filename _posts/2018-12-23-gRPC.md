...
결국 gRPC 에서 말하는 스트리밍이란 쉽게 말해서 http2 스트림을 사용한다는 의미입니다. gRPC의 스트리밍이 좋은 점은 병렬 처리를 쉽게 적용 할 수 있다는 점에서 매우 매력적입니다. 
...
(사실 어떻게든 단일 요청에서도 클라이언트 내부적으로 비동기로 구현해서 여러번 요청 해버리면 흉내가 가능하지만, 클라이언트의 로직 복잡도가 높아지는 단점이 있습니다.)
...
실제로 [http2 rfc](https://tools.ietf.org/html/rfc7540) 의 도입 부분을 보면 아래와 같은 말이 나오는 데, 위의 내용을 대입해보면 어떠한 의미를 가지는 지를 알 수 있습니다.

> This specification is an alternative to, but does not obsolete, the
   HTTP/1.1 message syntax.  HTTP's existing semantics remain unchanged.

> http2는 http1.1을 교체(대안) 하지 않으며, 기존 http의 의미를 깨지 않는다.
...
(또한 본문에서는 다루지 않지만, 양방향 스트리밍에 대한 예제도 repository에 같이 ([Simple proto](https://github.com/glqdlt/ex-grpc/blob/master/model/src/main/proto/Simple.proto)) 업로드 되어 있으니, 필요하신 분은 참고해주세요. 양방향 스트리밍 까지 서술하려면 내용이 너무 길어져서 설명은 생략하겠습니다 -_-;)
...
// message 키워드가 붙는 것은 Value Object 라고 보면 됨. 특이한 점은 필드의 순서를 정의할 수 있고 빌드 패턴으로 생성됨
message UserRequest {
    string id = 1;
}
...
proto 파일을 컴파일 하는 방법은 직접 컴파일러를 호출할 수도 있지만, 프로젝트 소스 툴에 플러그인을 추가하여 빌드 시에 자동으로 컴파일 될 수 있게도 할 수 있습니다.

저는 주로 프로젝트 툴로 Maven 을 사용하는 데, Maven 에는 protobug-maven-plugin 이 있습니다.
...
gRPC 의 Client 에서 중요한 개념은 stub 이라는 개념이 있습니다. TDD 를 하다 보면 마주치는 stub 이라는 개념과 비슷합니다. stub 은 canned answer 의 의미를 갖는 데, canned answer는 정해진 대답이란 의미를 갖습니다. 
