...
농단 반 섞인 이야기로 Restful API 대비 효율이 3배 좋다고 하는데요 -_-; 자세한 것은 [gRpc 성능 차트](https://cloud.google.com/blog/products/gcp/announcing-grpc-alpha-for-google-cloud-pubsub) 에서 정보를 얻을 수 있습니다.
...
여기서 단방향의 단일 call과 stream 의 차이는 뭘까요?
...
여담이지만 http2 를 보고 statefull 인지, stateless 인지에 대한 헷갈려 하는 토픽들을 많이 볼 수 있습니다. 기본적으로 http2 는 기존 http1.1 과 마찬가지로 restfull 개념을 승계하고 있으며 (stateless) 많이들 오해하는 http2 는 세션을 계속 물고 있을 것이라는 것은 조금 다릅니다. 
...
아래 예제에서는 일반적인 단방향 스트리밍을 예제로 진행해보겠습니다.
...
서버 푸시가 pub / sub 랑 동일한것줄 알았는 데..
http 2 에서의 server push 를 보면, 그게 아니라 단일 요청에 여러가지 응답을 보낼수 있다는 것이다. 그리고 이 요청은 keep alive 에 의해 유지될 뿐이고, 영원히 유지되지는 않는다.

즉 이게 뭔말이냐면..

기존에 server에서 특정 리소스와 데이터를 4번 보내려면  req <-> server 요청과 응답이 4번있어야 했다.

하지만 http 2 에서는

req -> server 1번 req <- server 4번 식으로 할 수 가 있어지는 것이다.

이걸 나는 pub / sub 처럼 생각했는 데

.onCompleted() 가 호출되면 이미 server 에서 푸시가 끝났다고 요청하는 꼴이기에

for(..){
    onNext() 로 여러번 호출하는 것이었다.
}
=======
...
>>>>>>> f65e2c93bcd9c4c8d4d652f667e3e50968d108b5
