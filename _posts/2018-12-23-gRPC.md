...
server 를 호출하는 client 는 간단하게 구현했음. 주의 깊게 볼 것은 gRPC 에서 remote method 의 응답값을 가져올 수 있는 방법이 3가지가 있는 데, 그 중에서 async 방법인 futureStub 을 사용했다는 점을 유의


- newBlockingStub() : 단방향 stub. 해당 메소드에서 block 이 일어남. 대부분 가장 친근하게 느낄 메소드
- newFutureStub() : 단방향 stub. async 하게 일어남 non-block.
- newStub() : 양방향 스트리밍을 지원하는 stub
