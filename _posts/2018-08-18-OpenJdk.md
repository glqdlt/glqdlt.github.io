...
과 같은 플랫폼 별 모델을 나누는 비지니스 전략을 취하면서 만들어진 이름입니다. 정확한 의미로는 자바 기본 사양이라고 볼 수 있고, 일반적으로 대중적인 의미로 자바 언어라고 하면 이 SE를 뜻합니다.  SE 는 일반 컴퓨팅의 목적을 의미하고, EE는 비지니스 서비스 환경, ME 는 임베디드 환경을 목적으로 합니다.
...
java ee 를 구현한 것으로는 대부분의 WAS(Web Application Server)가 해당합니다. 티맥스의 제우스나 Oracle 의 Webjogic, Redhat의 Jboss 등이 있습니다.  주변에서 흔하게 볼 수 있는 Tomcat 같은 WAS의 경우에는 서블릿 컨테이너로서의 역활만 할 뿐 Java EE 의 Full Spec 을 구현한 것이 아닙니다. 그래서 톰켓을 EE라 하지 않고 경량 컨테이너라고 부르기도 합니다.
...
위에서도 말했지만 대중적인 의미에서 Java SE 의 버전을 Java Version 과 동일 시 하여 불리곤 합니다. 딱히 자바 버전에 다른 명시를 하지 않은 경우에 java se 8 == jdk 8 == java 8 인 셈 됩니다.
