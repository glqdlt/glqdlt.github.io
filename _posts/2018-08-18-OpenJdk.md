...
아래에서 더 자세히 얘기하곘지만,  se 는 자바의 표준 spec 을 뜻 하는 것은 아니고, 자바를 개발한 썬에서 (지금은 Oracle 이 인수) standard edtion, enterprize edtion (ee), micro edtion(me) 과 같은 상업적 모델을 나누는 전략을 취하면서 만들어진 이름입니다. 자바의 상업적 모델의 default 가 되는 버전이 SE 인 셈이죠. ee는 엔터프라이즈 기능이 포함되어 있는 데, 대표적으로 웹 브라우저에 자바 어플리케이션을 실행하는 web start kit 과 같은 것들이 있었습니다, 또한 엔터프라이즈 서버 개발에서의 EJB와 같은 기능도 있습니다. me 는 개인적으로 많이 생소한데, 임베디드 OS로 구동 될 java 를 뜻 합니다. 과거에 소형 백색가전을 java 로 개발한 사례가 있었다고 합니다.

여기서 재밌는 점은 j2se에서의 java se로의 네이밍 변경이 일어난 부분입니다. 이에 대해서는 명확한 정보를 찾지 못했지만, 약간의 뇌피셜을 얹어 보면  java se 라 불리우게 된 시기는 java se6 부터로.. 이 때가 썬이 Oracle 에 인수 될 준비를 한 시기 이기도 합니다. (인수 발표 2009년, se6 개발 2006) 아마 Oracle 에서 버전 네이밍을 변경하길 원했던 게 아닐까 하는 생각을 해보고 있습니다.


## 한국과 OracleJDK

한국의 대부분 SI 시장에서는 Oracle 을 많이 사용합니다. DB 부터 코딩을 하는 자바 JDK 까지 Oracle 을 많이 사용하고 있습니다. 흔히 도장찍기라고 말하는 한국식 SI 자바 개발자를 양성하는 교육소에서도 가장 첫번째 수업으로 Oracle JDK 을 설치하는 과정을 배우기도 하구요. (저 역시 그랬음)
...
문제는 OpenJDK는 사용하기가 까다로운 놈입니다. OpenJDK 프로젝트는 소스코드만 개발하고, 따로 JDK를 빌드하고 쉽게 사용할 수 있는 패키지를 제공 하지 않기때문입니다. [official](http://openjdk.java.net/install/)를 보면 그나마 리눅스 설치 패키지 정도는 관리해주고 있지만, 윈도우나 맥 사용자는 개발자가 직접 소스코드를 받아서 빌드 해야합니다. 이러한 이유는 프로젝트에 포함 된 여러 자바 컴포넌트들이 다양한 라이센스와 비공개 유무로 복잡하게 엮이었기 때문이라고 합니다.
