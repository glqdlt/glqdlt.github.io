...
결론적으로 개발자 입장에서는 개발 pc에 윈도우에 설치된 OracleJDK를 제거하고 대안인 Azul 로 마이그레이션 하더라도 둘다 OpenJDK 를 base로 하는 태생이 같기 때문에 문제는 없습니다. 심지어 기술이사가 OracleJDK 의 일부 유료 컴포넌트들을 무료로 풀겠다고 선언헀으니 사실상 거의 동일합니다. (OracleJDK 에 포함된 모니터링 툴을 사용하는 사람이라면 대안을 찾아봐야겠지만요) 
