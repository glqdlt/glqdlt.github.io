...
재밌는 점은 이런 부분 때문에 JENKINS 의 경우에 Java 환경 변수를 셋팅하는 과정에서, JDK를 자동 설치하는 구문에 BCL 라이센스를 승인한 Oracle 사용자의 ID 와 PW 를 요구하는 부분이 있습니다. 이는 OracleJDK가 무료로 사용은 가능 하지만, 다운로드에 대해서는 1인에 한한 BCL 라이센스를 승인을 해야하기 때문에 그러한 것을 알 수 있습니다. BCL 에 대한 동의는 기입한 Oracle 의 ID 유저가 승인하게 되는 것이고, 승인을 바탕으로 다운로드 받은 OracleJDK 패키지를 개발 및 테스트를 목적으로 Jenkins 에서 사용하게 되는 것임으로 라이센스 위배가 되지 않습니다.

<img src="/images/zulu/jj.PND">
