...
따지고 보면 최근 MSA 패러다임이 확산화 되면서 어플리케이션이 서비스 관점에서 작은 컴포넌트로 구성되는 경우가 많아 이미 잦은 리팩토링을 권장하고 있던 분위기 이기도 합니다. 고로 잦은 메이저 버전 업그레이드에 대한 거부감은 크게 없을꺼란 생각을 가지고 있습니다. (어디까지나 제가 몸 담고 있는 웹 개발 영역에서의 이야기입니다 -_-;;)
