...
Oracle의 유료 정책을 논하기 전에 자바의 복잡해괴한 버전과 종류들에 대해서 먼저 얘기 할 필요가 있습니다. 우선 가장 첫번째로 가장 기본적인 Java 의 구성에 대해서 말해볼까 합니다.
...
앞에서 말씀드린 것처럼 Oracle 에서는 범용적인 컴퓨팅에서의 무료인 Java SE 와 비범용적인 (유료) Java SE 를 따로 구분해서 다운로드 페이지를 만들었습니다. 문제는 2019년 1월 부터는 유료 관련 다운로드나 업데이트를 위해서는 My Oracle Support(Oracle Subscription) 라는 별도의 서비스를 통해서 다운로드 할 수 있어진다고 합니다. 
...
지금 많이 사용 되는 Oracle SE 8 을 예로 들면 무료 사용자를 대상으로 하는 보안 업데이트는 2020년 까지 지원을 해줄 것이지만, 새로이 개발 되는 Java 11 버전 이후에는 BCL 라이센스가 아닌 Oracle Network License 로 갱신되고 유료 다운로드 형태로만 제공할 것이라는 점입니다. 결국 이는 Java 11 이후 유료로의 전환을 일컫습니다. (유료 사용자는 2019년 01월 부터 새로운 라이센스를 구독해야야 하고, 기존 유료 다운로드 페이지에서, 사용자 전용 다운로드 페이지를 제공합니다.)
...
프로그램 개발, 및 테스트, 프로토타이핑은 무료이지만, 상업적 생산적 비지니스 목적을 갖는 사용은 금지한다.
...
이러한 Oracle 의 정책 변화에 대해서는 아래의 이유를 생각해 볼 수 있습니다. 최근에 대부분의 서비스가 클라우드 기반으로 플랫폼이 대거 이동함에 따라 서버 N 당의 라이센스로 측정되던 요금이 가변형(오토스케일아웃 등)으로 들쑥날쑥 해지면서 Oracle이 손실을 보고 있었기에 강경하게 나오게 된 것이 아니었나 하는 생각을 하고 있습니다.
...
반면 OracleJDK 를 사용하면서 LTS 를 받고 있는 유료 구독자라면 Oracle 에서 제공하는 업데이트 패치만 적용 하면 됩니다. 이렇게 되면 소프트웨어에 손을 대지 않고도 해결할 수 있습니다.

결론적으로 보면 Oracle 은 무료인 OpenJDK 의 생명 주기를 짧게 만들어버려서, 비지니스 시장에서 유료 버전인 OracleJDK 의 사용을 반강제화 하려고 하는 의도를 알 수 있습니다.

그래도 사내에 개발 조직이 있으면서 서비스를 운영하고 있는 회사라면 대부분의 경우 (개발자들이 갈려나가겠지만 -_-;) OpenJDK를 도입해서 잦은 리팩토링을 통해 무료로 자바를 서비스 할 수 있습니다. 문제는 '잘 사용하고 있었는 데..' 의 경우로 개발 조직이 없는 경우에는 Oracle 이나 다른 유료 비지니스 플랜에 의지 할 수 밖에 없어졌습니다.
...
OracleJDK는 개발 환경에 관해서는 무료입니다. 올해까지 BCL 라이센스에 동의하고 합법적으로 받은 기존 OracleJDK는 그냥 사용해도 됩니다. 또한 현재 가장 인기 있는 OracleJDK SE 8 에 대해서는 2020년까지의 업데이트를 약속하였으니 업데이트 받아 사용해도 됩니다. 


문제는 OracleJDK 11 이후는 사실 상 유료가 되어버렸음으로, 대안으로 OpenJDK를 base로 하는 Zulu 등의 사용을 생각을 해보아야합니다. 다만 Zulu 이 녀석도 OpenJDK 를 base로 하기에 Azul 의 LTS 유료 플랜을 구매하지 않는 이상 보안 업데이트나 성능 업데이트라는 명목에 마이너 버전이 아닌 메이저 버전 업그레이드를 해주어야 합니다, 자바의 라이프사이클이 6개월로.. 짧아졌으니깐요. 
...
그리고 조금 다른 얘기지만 보안이 문제가 되는 JDK 설치 된 개발자 자신의 PC가 취약점으로 공격 당할 수 있다고 생각하면 -_-;; 이슈가 없더라도 자주 업데이트를 해주는 것이 좋습니다.


<img src ="/images/imo/sad.jpg"/>

따지고 보면 최근 MSA 패러다임이 확산화 되면서 어플리케이션이 서비스 관점에 작은 컴포넌트로 구성되는 경우가 많기 때문에 이미 잦은 리팩토링을 권장하고 있는 상황이며, 자바 버전 업그레이드를 하더라도 물리적인 작업이 조금 더 발생할 뿐임으로, 서비스 영속성에는 문제가 생기지 않을 것입니다.
...
결론적으로 개발자 입장에서는 개발 pc에 설치된 OracleJDK를 제거하고 대안인 Azul 로 마이그레이션 하더라도 태생이 같기 때문에 문제는 없습니다. 심지어 OracleJDK 의 일부 유료 컴포넌트들을 Openjdk에 무료로 포함해서 풀겠다고 선언헀으니깐요. 아무쪼록 OracleJDK의 빈 자리를 OpenJDK 가 잘 채워주길 바라면서, 별 일 없이 앞으로도 해피 코딩을 할 수 있는 환경이 되면 좋겠습니다.
