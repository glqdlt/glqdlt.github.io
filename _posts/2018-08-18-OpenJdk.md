...
자바에는 JDK 와 JRE 라는 형태로 배포가 되고 있습니다. JDK(Java Develop Kit) 란 이름에서 보이듯이, 말그대로 JDK는 자바 기반의 Application을 개발하는 데에 있어 필요한 개발키트 이고, JRE(Java Runtime Environment)는 자바를 실행하기 위한 플랫폼으로 보면 됩니다. JRE 안에는  JDK 안에는 JRE도 포함이 되어있음. 
