...
라 나오고, 해석해보면 범용적인 컴퓨팅에서는 무료라고 명시 되어 있습니다. 요 범용적인 컴퓨팅의 표현이 애매하지만, 임베디드 환경을 제외한 일반적인 컴퓨팅 작업을 말하는 것으로 이는 코딩이나 프로그래밍도 해당되는 내용입니다. 결론적으로 보면 프로그래밍 개발 환경에 대해서는 무료라는 말이 되고 이는 업무 PC에서도 같은 내용입니다.
