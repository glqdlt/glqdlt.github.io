...
JVM은 (자바) 소스코드로 부터 만들어지는 JVM 이 읽을 수 있는 (자바) 바이너리 파일인 .class 가 구동되는 플랫폼입니다. JVM 이 세상에 등장했을 당시 슬로건 처럼, 어떠한 OS 에서도 컴파일 된 Class 파일과 JVM이 설치만 되어 있으면 자바를 구동할 수 있습니다. 이런 형태 탓에 종종 자바에는 컴파일 언어이면서도 스크립트 파일과 같은 인터프리트 언어의 성격을 갖고 있다고도 책에서 소개 되곤 합니다.
재밌는 점은 이러한 성격 탓에 컴파일 언어인 자바는 디컴파일이 쉽게 되고, 더 나아가서 자바 외의 언어들이 JVM 위에서 구동되기도 합니다. 예를 들면 Groovy 나 Go, Gradle, 코틀린, 스칼라 등이 있겠습니다. Progarmming Syntax 가 바뀌더라도 결국 .class 파일로 컴파일만 되면 되는 거니깐요 -_-;
이런 이유로 제가 Java Virtual Machine 이라고 소개 하지 않고 (Java) Virtual Machine 이라고 타이틀을 적은 이유이기도 합니다.


## JRE ((Java) Runtime Enviroment )

JRE 는 이름 그대로 (자바) 실행 환경입니다. 웹서핑을 하려면 '웹 브라우저'를 설치해야 하는 것처럼 자바 어플리케이션을 실행 할 최소 요건입니다.
...
자바 컴파일러를 포함한 자바 application을 개발하기 위한 도구의 집합체. 개발에서 가장 중요한  클래스 컴파일러와 함께 자주 사용 되는 dump tool, 모니터링 툴들이 함께 KIT 화 된 것이 JDK 입니다.
...
Java SE(Standard Edition) 은 이름 그대로, 자바 표준을 뜻 합니다. 레퍼런스를 뜻 하는 표준이란 뜻은 아니고.. 강, 중, 약 에서 중간을 의미합니다. 이게 무슨 말이냐면, Java 의 버전 히스토리를 보면 자바에는 Java Enterprise Edition (JAVA EE), Java Micro Edition (JAVA ME) 와 같이 기능이 구별 된 버전들이 여럿 존재하고 있습니다. Java EE 는 엔터프라이즈 기반의 서버 기능들이 모두 내장되어 있고, Java ME 는 모바일 장치 및 임베디드와 같은 제한적인 소규모 장치용 기능들이 제공 된 사양이었습니다. SE는 이 중간 다리가 되는 것이고, 이런 이유로 강,중,약 이라고 표현했던 이유입니다. 자세한 것은 [WIKI](https://en.wikipedia.org/wiki/Java_Platform,_Standard_Edition) 를 보면 재미난 이야기가 나옵니다.


## OpenJDK

OpenJDK는 오라클과 오픈소스 진영에서 합작으로 진행 하는 자바 SE 의 구현체입니다([공식](http://openjdk.java.net/faq/)).오픈 소스 진영에서 만들어지다 보니 무료로 사용이 가능하고, GNU 라이센스가 부여되어 있습니다. 재밌는 것은 이 OpenJDK는 일종의 프로젝트이며, 이 프로젝트 하위에 포함되는 것은 아래와 같습니다.
...
결국 우리가 사용 되는 자바의 모든 것을 개발한다고 보면 됩니다. 여기서 조금 혼선이 오는 데 OpenJDK는 현재 표준처럼 사용되곤 하지만, ME,SE,EE 등이 있는 것을 생각해보면 Java SE 의 스펙에 대한 구현체일 뿐 임을 망각할 수도 있습니다.
 ...
 OracleJDK는 이 OpenJDK 의 소스코드를 기반으로 Oracle 의 엔터프라이즈 기능들과 튜닝된 기능들이 담겨서 OracleJDK 로 만들어집니다.


OpenJDK는 뭐랄까 사용하기가 여간 까다로운 놈입니다. [official](http://openjdk.java.net/install/)를 보면 openjdk는 리눅스 실행 패키지만 직접 관리 배포하고 있어서 윈도우나 맥 사용자에게는 껄끄럽습니다.

윈도우 환경에서는 [ojdkbuild](https://github.com/ojdkbuild/ojdkbuild) 오픈 프로젝트나, [zulu](https://www.azul.com/downloads/zulu/) 클라이언트 .msi 를 받아서 설치하는 식으로 해결할 수 있습니다.
...
자바에서 Java SE는 표준 스펙인 JSR(Java Spec Request)를 토대로 정의 된 것을 뜻한다. 정의 된 Java SE는 그를 구현하는 벤더들에 의해 개발되고 JKT 인증으로 '우앙,ㅋ 이건 잘만든 자바 구현체임' 이란 도장을 찍어준다. 그런데 병크로 Java의 7버전인 JDK7는 이 Java SE7가 확정되기 전에 개발이 진행 되었다고 한다[(OpenJDK 위키)](https://ko.wikipedia.org/wiki/OpenJDK). 

표준 스펙이 완성되기 전에 배포가 됬다는 이야기인데, 스펙이 정해지기 전에 개발 먼저 하고 배포되었으니 이런 저런 문제가 많이 발생했다. 이런 흑역사 때문에 당시에 많은 문제가 있었다고 한다. 구글링 해보면 native I/O 기능 쪽에서 특히 문제가 많았다고 한다.
