...
자바에는 JDK 와 JRE 라는 형태로 배포가 되고 있습니다. JDK(Java Develop Kit) 란 이름에서 보이듯이, 말그대로 JDK는 자바 기반의 Application을 개발하는 데에 있어 필요한 개발키트 이고, JRE(Java Runtime Environment)는 자바를 실행하기 위한 플랫폼으로 보면 됩니다.

## JVM ( (Java) Virtual Machine )

JVM은 (자바) 소스코드로 부터 만들어지는 JVM 이 읽을 수 있는 (자바) 바이너리 파일인 .class 가 구동되는 플랫폼입니다. 그런데 자바 버추얼 머신이라고 안하고 왜 ? (자바) 버추얼머신이라고 돌려서 썼는 지는, 최근 JVM 기반의 다양한 언어들이 자바 그 이상의 인기를 끄는 경우도 있기에 굳이 자바 한정적으로만 얘기하면 안되겠다는 생각에서였기에 (자바) 버추어 머신이라고 표기함.


## JRE ( (Java) Runtime Enviroment )

JRE 는 이름 그대로 (자바) 실행 환경입니다. 웹서핑을 하려면 '웹 브라우저'를 설치해야 하는 것처럼 자바 어플리케이션을 실행 할 최소요건입니다.
JRE가 JVM 자체가 아니냐고 물어보면 반은 맞고 반은 틀리다고 생각합니다.(따지고 보면 맞습니다만) 정확히는 JVM 만 포함된 것이 아니고, 다양한 OS 에서 구동할 수 있도록 JVM Core 를 보좌하는 실행 환경 라이브러리들을 포함한 것이 JRE 입니다. (윈도우 JRE나 리눅스 JRE의 경우 이 JRE의 라이브러리 구성요소가 다름. JVM core 가 달라지지는 않겠지만)
...
자바 컴파일러를 포함한 자바 application을 개발하기 위한 도구의 집합체. 현업에서 자주 사용 되는 dump tool 과 같은 도구들과 함께, 개발에서 가장 중요한 자바 컴파일러가 이 JDK에 포함 됩니다.
...
자바에서 Java SE는 표준 스펙인 JSR를 토대로 정의 된 것을 뜻한다. 정의 된 Java SE는 그를 구현하는 벤더들에 의해 개발되고 JKT 인증으로 '우앙, 너 잘만들었음' 이란 도장을 찍어준다. 그런데 희대의 병크로 Java의 7버전인 JDK7는 이 SE가 확정되기 전에 개발이 진행 되었다고 한다[(OpenJDK 위키)](https://ko.wikipedia.org/wiki/OpenJDK). 

표준 스펙을 완성하기 전에 개발이 됬다는 이야기인데, 이 JDK7의 모체가 되는 것이 openjdk6 였다. 그러니깐 openjdk6 에서 Java 7 버전의 베타 개발을 같이 하고 java se 7이 만들어지기 전에 JDK7을 만들어 버린 것이다. Se가 먼저 나오고 JDK 가 나오는 것인데, 뭐랄까 달걀에서 닭이 튀어나오는 느낌이랄까

이런 흑역사 때문에 OpenJDK6 는 많은 문제가 있었다고 합니다. 구글링 해보면 특히 native I/O 기능 쪽에 문제가 많았다고 함. 
