...
앞에서 말씀드린 것처럼 Oracle 에서는 범용적인 컴퓨팅에서의 무료인 Java SE 와 비범용적인 (유료) Java SE 를 따로 구분해서 다운로드 페이지를 만들었습니다. 문제는 2019년 1월 부터 나누어서 구분하던 것을 무료 다운로드 페이지 하나로 합치기로 하고, 유료 다운로드나 업데이트를 위해서는 My Oracle Support(Oracle Subscription) 라는 별도의 서비스를 통해서 다운로드 할 수 있어진다고 합니다. 그리고 무료 버전 공개 다운로드 페이지는 2020년 이후는 closing 될 예정입니다.
...
사실상 무료(공개 업데이트)로서의 종료를 알리는 의미인 것을 알 수 있습니다. 지금 많이 사용 되는 Oracle SE 8 을 예로 들면 무료 사용자를 대상으로 하는 보안 업데이트는 2020년 까지 지원을 해줄 것이지만, 새로이 개발 되는 Java 11 버전 이후에는 유료 다운로드로 전환할 것이라는 점입니다. 결국 이는 완전 유료로의 전환을 일컫습니다. (유료 사용자는 2019년 01월 부터 새로운 라이센스를 구독해야야 하고, 기존 유료 다운로드 페이지에서, 사용자 전용 다운로드 페이지를 제공합니다.)
...
사내 외에서의 프로그램 개발, 및 테스트, 프로토타이핑은 지금까지도 무료이지만, 상업적 생산적 비지니스 목적을 갖는 사내에서의 사용은 금지한다.
