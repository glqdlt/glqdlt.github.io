...
se 는 자바의 기준 이 되는 의미를 뜻하는 스펙입니다, 자바를 개발한 썬에서 (지금은 Oracle 이 인수) 
...
과 같은 비지니스 기능 별 모델을 나누는 전략을 취하면서 만들어진 이름입니다. 정확한 의미로는 자바 기본 사양이라고 볼 수 있겠습니다. 자바 자체에 대한 표준은 [JSR(Java Spec Request)](https://jcp.org/en/jsr/overview) 에서 정의를 하고 있고, SE는 JSR 에서 기본 사양에 해당하는 기능들을 묶은 Spec 입니다.
java se 를 구현한 것으로는 openjdk 가 대표적이고 이를 base로 하는 OaacleJDK, Zulu JDK 등이 있습니다.

ee는 엔터프라이즈 기능이 포함되어 있는 기능들에 대한 spec 이 정의 된 모델로, 대표적으로 웹 브라우저에 자바 어플리케이션을 실행하는 web start kit 과 같은 것들이 있었습니다, 또한 엔터프라이즈 서버 개발에서의 EJB와 같은 기능도 있습니다. 

java ee 를 구현한 것으로는 대부분의 WAS(Web Application Server)가 해당합니다. 티맥스의 제우스나 Oracle 의 Webjogic 등이 있습니다. 주변에서 흔하게 볼 수 있는 Tomcat 같은 WAS의 경우에는 서블릿 컨테이너로서의 역활만 할 뿐 Java EE 의 Full Spec 을 구현한 것이 아닙니다. 그래서 톰켓을 보고 경량 컨테이너라는 말이 나오기도 합니다.
...
OracleJDK 에는 OpenJDK 에 없던 상용 기능이 포함되었으나, 앞으로 OpenJDK 에는 OracleJDK 에 있던 아래 일부 기능들을 제공한다.
...
그리고 [Oracle JDK 11](https://www.oracle.com/technetwork/java/javase/terms/license/javase-license.html) 다운로드 페이지에는 기존 BCL 라이센스에서 새로 갱신 된 Oracle Network License로 변경된 것을 알 수 있습니다.
...
프로그램 개발, 및 테스트, 프로토타이핑은 앞으로도 무료이지만, 상업적 생산적 비지니스 목적을 갖는 사용은 금지한다.

즉, 사실상 OracleJDK 유료화 선언인 것을 알 수 있습니다.
...
이러한 Oracle 의 정책 변화에 대해서는 아래의 이유를 생각해 볼 수 있습니다. Oracle의 입장은 언제나 비슷했는 데, 최근에 대부분의 클라우드 기반으로 서비스 플랫폼 등이 대거 이동함에 따라 서버 N 당의 라이센스로 측정되는 요금이 가변형(오토스케일아웃 등)으로 들쑥날쑥 해지다보니 Oracle이 손실을 보고 있었기에 강경하게 나오게 된 것이 화근이었다고 하는 이야기가 있습니다. 
...
OracleJDK는 개발 환경에 관해서는 무료입니다. 올해까지 BCL 라이센스에 동의하고 합법적으로 받은 OracleJDK는 그냥 사용해도 됩니다. 또한 현재 가장 인기 있는 OracleJDK SE 8 에 대해서는 2020년까지의 업데이트를 약속하였으니 업데이트 받아 사용해도 됩니다. 


문제는 OracleJDK Se 8 이후는 사실 상 유료가 되어버렸음으로, 대안으로 Zulu를 대부분 사용하는 등의 생각을 해보아야합니다. 다만 Zulu 이 녀석도 OpenJDK 를 base로 하기에 Azul 의 LTS 유료 플랜을 구매하지 않는 이상 보안 업데이트나 성능 업데이트라는 명목에 마이너 버전이 아닌 메이저 버전 업그레이드를 해주어야 합니다, 자바의 라이프사이클이 6개월로.. 짧아졌으니깐요. 
...
결론적으로 개발자 입장에서는 개발 pc에 윈도우에 설치된 OracleJDK를 제거하고 대안인 Azul 로 마이그레이션 하더라도 둘다 OpenJDK 를 base로 하는 태생이 같기 때문에 문제는 없습니다. 심지어 기술이사가 OracleJDK 의 일부 유료 컴포넌트들을 무료로 풀겠다고 선언헀으니깐요. (OracleJDK 에 포함된 모니터링 툴을 사용하는 사람이라면 대안을 찾아봐야겠지만요) 
