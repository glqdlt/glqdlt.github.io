...
- 추상 클래스인 SomeLog  를 저장하고 싶다면 몽고DB에 저장은 된다. 다만 이후에 저장된 것을 다시 자바로 읽어들이는 과정에서 추상클래스를 구현한 익명 클래스의 인스턴스로 매핑을 시도하게 되는 데, 추상클래스의 추상 메소드를 익명클래스에서 어떻게 구현해야 할 지 판단이 안 서기 때문에 매핑이 불가능하다는 에러를 내뿜는다.
